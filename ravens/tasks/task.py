...
class Task():
  ...
  def _continuous_oracle(self, env, **kwargs):
    ...
    This oracle will generate the pick and place poses using the original discrete
    oracle. It will then interpolate intermediate actions using splines.
    """
    kwargs['env'] = env
...
class ContinuousOracle:
  ...
  def act(self, obs, info):
    if not self._actions:
      ...
      self._actions = self._planner(self._env.get_ee_pose(), act['pose0'], act['pose1'])
